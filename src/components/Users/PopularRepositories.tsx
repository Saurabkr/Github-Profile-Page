...
const PopularRepositories: React.FC = () => {
  ...
  return (
    <div className="mb-6">
      <h2 className="text-base font-normal text-[#1f2937] mb-3">
        {repoTexts.title}
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {repositories.map((repo) => (
          <div
            key={repo.name}
            className="p-4 border border-[#dedee3] rounded-md transition-colors"
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <span className="ml-2 px-2 py-0.5 text-xs border border-[#30363d] rounded-full text-[#7d8590] flex-shrink-0">
            ...
            {repo.description && (
              <p className="text-xs text-[#7d8590] mb-3 line-clamp-2">
                {repo.description}
              </p>
            )}
