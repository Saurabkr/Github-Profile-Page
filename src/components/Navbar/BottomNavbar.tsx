...
const BottomNavbar: React.FC = () => {
  ...
  return (
    <nav className="border-[#30363d] lg:-mx-8 px-4 lg:px-8 w-full">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-2 px-4 py-3 border-b-2 text-sm transition-colors ${
                isActive
                  ? "border-[#f78166] text-black font-semibold"
                  : "border-transparent text-black hover:border-[#878a8f]"
              }`
            }
          >
            {tab.icon}
            ...
            {tab.count !== undefined && (
              <span className="px-1.5 py-0.5 text-xs bg-[#dadadb] text-black rounded-full min-w-[20px] text-center">
                {tab.count}
              </span>
            )}
          </NavLink>
